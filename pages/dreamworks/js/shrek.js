	...
	function visual_Slides(_targetWrap, _slides, _has_pause, _type){
		...
		if(!isMobile){
			$(window).on('mousewheel', function(event) {
				if (event.originalEvent.wheelDelta > 50 && scrollEvent == false && cnt >= 0) {
		...
		function slide_img(num, transition_val){
			...
			setTimeout(function(){scrollEvent = false;}, 500);
		}
...
/*~~~~~~~~~~~~~~~~~~~~ charater rolling ~~~~~~~~~~~~~~~~~~~~*/
	...
	/*function visual_Slides(_targetWrap, _slides, _has_pause, _type){
		var $wrap = _targetWrap;
		var $slides = $($wrap +" "+_slides);
		var $pn_btns = $($wrap +" div.view_mask > button");
	...
	function rolling_slide(_targetWrap, _view_ea_D, _view_ea_T, _view_ea_M){
		var $wrap = _targetWrap;
		var $inner = $($wrap +" div.inner_wrap");
		var $inner_ul = $($wrap +" div.inner_wrap > div.ul_wrap");
		var $inner_ul_li = $($wrap +" div.inner_wrap > div.ul_wrap > div.char");
		var $btn_prev = $($wrap +" button.prev");
		var $btn_next = $($wrap +" button.next");
		...
		function move_ul(_direction){
			...
			var idx = $($wrap).find("img").attr("src");
			console.log(idx);
			if(click_Event){
				click_Event = false;
				$inner_ul.animate({ left: (li_width * num) * move_cnt +"px" }, duration, function(){
					for(var i = 1; i <= move_cnt; i++){
						if(_direction == "prev"){
							$($wrap +" > div.inner_wrap > div.ul_wrap > div.char").last().prependTo($wrap +" > div.inner_wrap > div.ul_wrap");
						}//처음 순서만 기억하기 때문에 다시 선택 잡아줘야함
						else if(_direction == "next"){
							$($wrap +" > div.inner_wrap > div.ul_wrap > div.char").first().appendTo($wrap +" > div.inner_wrap > div.ul_wrap");
						}
					}
	...
	var movie_btn = $("div.info > div.inner_wrap > ul > li > button");
	var movie_view = $("div.info > div.inner_wrap > ul > li > img");
	
	movie_btn.click(function(){
		$(this).css("display", "none");
		movie_view.eq(movie_btn.index($(this))).css("display", "none");
	});
	...
	/*var charater_slide = visual_Slides(".ul_wrap", ".char", false, "slide");*/
	var charater_slide = rolling_slide(".info", 1, 1, 1);
	var section_slide = visual_Slides(".view", ".view_wrap", false, "slide");
